import type { RouteAnalysis, ComponentTreeNode } from "./types.js";
...
function renderTree(nodes: ComponentTreeNode[], depth = 0): string {
  return nodes
    .map((node) => {
      ...
      const childrenHtml = hasChildren ? `<div class="children">${renderTree(node.children, depth + 1)}</div>` : "";
...
export function generateHtml(data: RouteAnalysis): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .node { margin: 2px 0; border-left: 2px solid #30363d; margin-left: 8px; }
    ...
    .badge { font-size: 0.65rem; padding: 2px 6px; border-radius: 4px; text-transform: uppercase; font-weight: 600; }
    ...
    .props { font-size: 0.75rem; color: #8b949e; padding: 4px 12px 8px 32px; }
...
<body>
  ...
  <div class="stats">
    <div class="stat"><span class="stat-value">${data.stats.totalComponents}</span><span class="stat-label">Total Components</span></div>
    <div class="stat"><span class="stat-value">${data.stats.clientComponents}</span><span class="stat-label">Client Components</span></div>
    <div class="stat"><span class="stat-value">${data.stats.serverComponents}</span><span class="stat-label">Server Components</span></div>
    <div class="stat"><span class="stat-value">${data.stats.uniqueHooks.length}</span><span class="stat-label">Unique Hooks</span></div>
  </div>
  ...
  <div class="tree">${renderTree(data.componentTree)}</div>
  <script>
    function expandAll() { document.querySelectorAll('.node').forEach(n => n.classList.remove('collapsed')); }
    function collapseAll() { document.querySelectorAll('.node').forEach(n => { if (n.querySelector('.children')) n.classList.add('collapsed'); }); }
    collapseAll();
