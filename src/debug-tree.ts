...
async function debugTree(targetPath: string, route: string) {
  ...
  const componentMap = new Map<
    string,
    { name: string; jsxUsage: ReturnType<typeof extractJsxUsage> }
  >();
  ...
  for (const [file, data] of componentMap) {
    ...
    if (isRelevant) {
      ...
      console.log(
        `   Direct children: [${data.jsxUsage.directChildren.join(", ")}]`
      );
  ...
  if (specimenGridFile && specimenCardFile) {
    ...
    for (const [file, data] of componentMap) {
      const hasGridDirect =
        data.jsxUsage.directChildren.includes("SpecimenGrid");
      const nestedInGrid =
        data.jsxUsage.nestedInComponent.get("SpecimenGrid") || [];
      ...
      if (hasGridDirect && hasCardNestedInGrid) {
        ...
        console.log(
          `   SpecimenCard is correctly detected as nested inside SpecimenGrid!`
        );
      }
    }
  }
  ...
  for (const [file, data] of componentMap) {
    if (
      data.jsxUsage.directChildren.includes("SpecimenGrid") ||
      Array.from(data.jsxUsage.nestedInComponent.keys()).includes(
        "SpecimenGrid"
      )
    ) {
        ...
        if (matches) {
          for (const match of matches) {
            if (match.includes("SpecimenCard")) {
              console.log(
                `📍 In ${file}, SpecimenCard IS NESTED inside SpecimenGrid tags:`
              );
              console.log(
                "   " + match.slice(0, 200).replace(/\n/g, "\n   ") + "..."
              );
              console.log("");
            }
          }
